...
public class Soporte extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Soporte frame = new Soporte(c, null);
	 ...
	 * Create the frame.
	 */
	public Soporte(Clip c, String name) {
		setResizable(false);
		...
		btnAtras.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Continuar con = new Continuar(c, name);
				con.setVisible(true);
				dispose();
			}
		});
		...
		lblNewLabel.setBounds(343, 11, 439, 44);
		contentPane.add(lblNewLabel);

		JLabel textCompras = new JLabel("");
		textCompras.setBounds(32, 272, 1026, 65);
		...
		compras.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (compras.isSelected()) {
					...
					textCompras.setText(
							"<html>¿He comprado un artículo por accidente, que debo hacer? Para seguir viendo este problema, tendrá que compartir una captura de pantalla de la factura que se le entregó cuando realizó la compra, así como su ID de jugador en support@sink&win.com. Tan pronto como el equipo reciba los detalles, el equipo volverá a los siguientes pasos."
		...
		compras.setBounds(32, 234, 200, 40);
		contentPane.add(compras);

		JLabel textNuevoEnElJuego = new JLabel("");
		textNuevoEnElJuego.setBounds(32, 389, 1026, 65);
		...
		nuevoEnElJuego.setBounds(32, 348, 200, 40);
		nuevoEnElJuego.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (nuevoEnElJuego.isSelected()) {
					...
					textNuevoEnElJuego.setText(
							"<html>¿Cómo charlas con otras personas? Ve a opciones. Asegúrate de que el chat del juego esté marcado para poder chatear con otras personas <br>"
		...
		textArticulosPopulares.setBounds(32, 132, 1026, 91);
		...
		articulosPopulares.setBounds(32, 91, 200, 40);
		articulosPopulares.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (articulosPopulares.isSelected()) {
					...
					textArticulosPopulares.setText(
							"<html>¿Por qué no tengo la nueva actualización? <br> Gracias por informarnos sobre su interés en la nueva actualización.Entendemos que los jugadores sin la actualización estén ansiosos por acceder a ella.Actualmente la nueva actualización se encuentra en la fase de prueba beta y apreciamos su entusiasmo"
		...
		textProblemas.setBounds(32, 503, 1026, 65);
		...
		solucionDeProblemas.setBounds(32, 465, 200, 40);
		solucionDeProblemas.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (solucionDeProblemas.isSelected()) {
					...
					textProblemas.setText(
							"<html>Mi juego tarda demasiado en cargarse. Puedes probar estas soluciones si el juego tarda demasiado en cargarse: 1. Verifique si tiene una conexión a Internet estable y el dispositivo está conectado correctamente 2. Intente no cambiar entre la red Wifi y la red móvil mientras carga el juego 3. Si el almacenamiento de su dispositivo está lleno o casi lleno, intente liberar espacio y luego inicie el juego<html>");
				} else {
		...
		textComoSeJuega.setBounds(32, 620, 1026, 65);
		...
		comoSeJuega.setBounds(32, 578, 200, 40);
		comoSeJuega.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (comoSeJuega.isSelected()) {
					textComoSeJuega.setForeground(Color.BLACK);

					textComoSeJuega.setText(
							"<html>¿Por qué no obtuve las recompensas del último torneo? Tienes que reclamar tu premio ganador dentro de los próximos 7 días. Si no ha reclamado el premio, no recibirá las recompensas.<br>"
